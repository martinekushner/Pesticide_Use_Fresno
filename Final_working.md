...
```
    ...
    346623        10  M11S   13E      07  CENTRAL VALLEY          M      11S   
                 ...
                 MTRS      MTR      CO_MTRS  REGION     CO_MTR  County_CDn  \
...
def convert_sq_miles_to_sq_degrees(sq_miles):
    ...
    # The exact conversion would depend on the latitude, but for simplicity, we'll use an average conversion factor
...
# Plot the shapefile
pur_centroids.plot(ax=ax, color='lightgrey')

# Add centroid points with rectangular markers
for idx, row in pur_centroids.iterrows():
    centroid = row['geometry']
    sq_miles = row['AMOUNT_SQM'] 
    sq_degrees = convert_sq_miles_to_sq_degrees(sq_miles)
    side_length = np.sqrt(sq_degrees)  # The side length of the square in degrees
    
    # Create a rectangular marker
    rect = Rectangle((centroid.x - side_length / 2, centroid.y - side_length / 2), side_length, side_length, edgecolor='blue', facecolor='red', alpha=0.5)
    
    # Add the rectangle to the plot
    ax.add_patch(rect)

# Set plot title and labels
ax.set_title('Locations of Pesticide Applications in Fresno, 2021')
...
# Load the base shapefile
fresno_gdf = gpd.read_file("data/Fresno_County_PLSS_NAD83AlbersCA/Fresno_County.shp")
...
pur_centroids.plot(ax=ax, color='lightgrey', edgecolor='black')

# Add centroid points with rectangular markers for the primary shapefile
for idx, row in pur_centroids.iterrows():
    centroid = row['geometry']
    sq_miles = row['AMOUNT_SQM']  # Replace with the column you want to use for size
    sq_degrees = convert_sq_miles_to_sq_degrees(sq_miles)
    side_length = np.sqrt(sq_degrees)  # The side length of the square in degrees
...
fresno_gdf.plot(ax=ax, color='none', edgecolor='green')

# Set plot title and labels
ax.set_title('Locations of Pesticide Applications in Fresno, CA (2021)')
ax.set_xlabel('Longitude')
ax.set_ylabel('Latitude')
...
# Plot the primary shapefile
pur_centroids.plot(ax=ax, color='orange', edgecolor='yellow')

# Add centroid points with rectangular markers for the primary shapefile using precomputed values
for idx, row in pur_centroids.iterrows():
    centroid = row['geometry']
    sq_degrees = row['sq_degrees']
    side_length = np.sqrt(sq_degrees)  # The side length of the square in degrees
...
# Plot the additional shapefile
fresno_gdf.plot(ax=ax, color='none', edgecolor='grey')
...
# Define custom colormaps for shades of blue and pink
blue_colors = [(0, (0.8, 0.8, 1)), (1, (0, 0, 0.5))]  # Light blue to dark blue
pink_colors = [(0, (1, 0.8, 0.8)), (1, (0.5, 0, 0))]  # Light pink to dark pink

# Ensure both colormaps have the same number of color values
num_colors = max(len(blue_colors), len(pink_colors))
blue_colors = blue_colors * (num_colors // len(blue_colors))
pink_colors = pink_colors * (num_colors // len(pink_colors))
...
# Plot variable1 (shades of blue)
ces_gdf.plot(column='pestP', cmap=blue_cmap, legend=True, ax=ax, linewidth=0.8, edgecolor='k')

# Plot variable2 (shades of pink)
ces_gdf.plot(column='lingP', cmap=pink_cmap, legend=True, ax=ax, linewidth=0.8, edgecolor='k')
...
```
    ...
    Cell In[187], line 15
         12 blue_colors = blue_colors * (num_colors // len(blue_colors))
         13 pink_colors = pink_colors * (num_colors // len(pink_colors))
    ---> 15 blue_cmap = LinearSegmentedColormap.from_list('blue_cmap', blue_colors)
         16 pink_cmap = LinearSegmentedColormap.from_list('pink_cmap', pink_colors)
         18 # Plot the bivariate choropleth map
    

    File ~\anaconda3\Lib\site-packages\matplotlib\colors.py:1067, in LinearSegmentedColormap.from_list(name, colors, N, gamma)
...
conf['plot_title'] = 'Texas housing and population'
...
conf['map_zoom'] = 5  # Zoom factor of the map
conf['hover_x_label'] = 'Housing ratio'  # Label to appear on hover
conf['hover_y_label'] = 'Population ratio'  # Label to appear on hover

# Define settings for the legend
conf['line_width'] = 0.5  # Width of the rectagles' borders
conf['legend_x_label'] = 'Higher housing ratio'  # x variable label for the legend 
conf['legend_y_label'] = 'Higher pop ratio'  # y variable label for the legend
# Create our bivariate map
fig = create_bivariate_map(df, color_sets['pink-blue'], geojson, x='pop_2019', y='cumulated_pop', ids='geo', name='nuts_name', conf=conf)
...
# Plot the choropleth map
fig, ax = plt.subplots(figsize=(10, 8))
ces_gdf.plot(column='pestP', cmap='Reds', label='Pesticide Use', legend=True, alpha=.5, ax=ax)
ces_gdf.plot(column='lingP', cmap='GnBu', label='Linguistic Isolation', legend=True, alpha=.5, ax=ax)
...
# Plot variable2 using shades of blue
for idx, row in ces_gdf.iterrows():
    color = pink_blue_cmap_var2(row['asthmaP'])
...
# Add a title
plt.title('Bivariate Map: Correlation of Pesticide Use and Asthma Rates')
...
# Plot variable1 using shades of pink
ces_gdf.plot(column='pestP', cmap=pink_blue_cmap_var1, legend=False, ax=ax, linewidth=0.8, edgecolor='k')

# Plot variable2 using shades of blue
for idx, row in fresno_data.iterrows():
    color = pink_blue_cmap_var2(row['normalized_variable2'])
    ax.add_patch(row['geometry'].__geo_interface__, facecolor=color, linewidth=0)

# Create a custom legend with a 3x3 grid of colors and labels
legend_elements = []
for i, color in enumerate(pink_blue_colors):
    legend_elements.append(Patch(facecolor=color, edgecolor='k', label=f'Label {i+1}'))
